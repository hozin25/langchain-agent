...
- LLM selection is `createLlm(modelId)` in `llm.ts`. GLM (智谱, OpenAI-compatible) is the default; DeepSeek is the alternative, both via `ChatOpenAI`. The model list shown in the UI comes from `listModels()`. `streaming` is `false` but this is now conservative rather than load-bearing: token streaming flows through LangGraph's `streamMode: 'messages'`, which drives ChatOpenAI's new `_streamChatModelEvents` path (via core's `convertOpenAICompletionsStream`, independent of the `streaming` flag) and correctly separates `reasoning_content` from the final answer. The historical bug, `streaming: true` dropping the post-tool-call answer (mis-roled as a generic `ChatMessage`, surfaced as "No response received"), was verified fixed in `@langchain/openai` 1.5.5; reproducer in `scripts/probe-glm-stream.cjs`.
